...
- 本地调试可使用 headless 运行方式，只监听指定端口，不打开 native UI。
...
7. 前端从当前页面 origin 推导同端口 `/ws` RPC 地址并建立连接。
8. 前端通过 `app.hello` handshake 获取应用能力、版本、权限和初始会话状态。
...
3. App Host 提供 Web 页面、health endpoint 和 RPC WebSocket endpoint。
4. App Host 不打开 native UI surface。
5. 调试前端、浏览器或自动化测试连接该显式端口。
...
第一阶段本地开发使用 localhost HTTP/WebSocket。对外入口约定为 Web 页面和 RPC WebSocket 同 origin，RPC 固定路径为 `/ws`。dev 模式下 Vite 可以作为浏览器入口，并把 `/ws` 转发到 Go service；正式 exe 模式下 Go service 直接服务前端静态资源和 `/ws`。后续本地桌面场景可增加低开销 IPC Transport，例如命名管道或其他 Windows IPC。远程浏览器使用 HTTPS/WSS。
